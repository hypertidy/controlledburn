...
This is *exactly* what we need. However:
...
Start with **Phase 1, Task 1.1**: clone exactextract, inspect `src/` for the minimal headers needed. The key question is how tightly `raster_cell_intersection` (the coverage fraction entry point) is coupled to the rest of the framework. If it's clean enough to call with just a `Grid` + `GEOSGeometry*`, vendoring is viable. If it pulls in too much, we reimplement the algorithm.

**Also worth noting**: Dan Baston is contributing rasterization directly into GEOS core ([PR #1232](https://github.com/libgeos/geos/pull/1232), [PR #1295](https://github.com/libgeos/geos/pull/1295)). If/when that lands, the entire exactextract algorithm becomes available via the geos package directly, which could simplify or even replace the vendored C++ approach. Worth tracking but not blocking on.
